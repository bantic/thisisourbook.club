...
function setImageHeights() {
  ...
  $(".meeting__img--img").each(function() {
    ...
    $(this)
      .parent(".meeting__img--wrapper")
      .css({ height: height - wiggle + "px" });
    $(this)
...
function setupOpacityIncreasers() {
  ...
  inView(selectors.enter).on("enter", function(el) {
    setTimeout(function() {
...
function setupFadeIns() {
  ...
  inView(selectors.wrapper).on("enter", function(el) {
    $(el)
      .find(selectors.inner)
      .delay(delay)
      .fadeIn(duration);
  });
}
...
function setupLineAnimations() {
  ...
  inView(selectors.bookIcon).on("enter", function(el) {
    if (
      el.classList.contains(cssClasses.doneAnimating) ||
    ...
    new Vivus(el, { file: bookLinesPath }, function _doneAnimating() {
